...
def index(m3u8):
    ...
    source = m3u8
    source = source.replace('https://orca-app-rzon8.ondigitalocean.app/', '')
    ...
    tsal = ts.text
    tsal = tsal.replace(videoid+'_','https://orca-app-rzon8.ondigitalocean.app/getstream?param=getts&source=https://deneme-1e010.kxcdn.com/hls-live/'+videoid+'/1/'+videoid+'_')
    if "internal" in tsal:
        tsal = tsal.replace('internal','https://orca-app-rzon8.ondigitalocean.app/getstream?param=getts&source=https://deneme-1e010.kxcdn.com/hls-live/'+videoid+'/1/internal')
    if "segment" in tsal:
        tsal = tsal.replace('\n'+'media','\n'+'https://orca-app-rzon8.ondigitalocean.app/getstream?param=getts&source=https://deneme-1e010.kxcdn.com/hls-live/'+videoid+'/1/media')
    return tsal

@app.route('/getm3u8',methods=['GET'])
def getm3u8():
    source = request.url
    source = source.replace('https://orca-app-rzon8.ondigitalocean.app/getm3u8?source=', '')
    source = source.replace('%2F', '/')
    source = source.replace('%3F', '?')
    headers = {
        "accept": "*/*",
...
def getstream():
    ...
    if param == "getts":
        source = request.url
        source = source.replace('https://orca-app-rzon8.ondigitalocean.app/getstream?param=getts&source=','')
        source = source.replace('%2F','/')
        source = source.replace('%3F','?')
        headers = {
            'accept': '*/*',
